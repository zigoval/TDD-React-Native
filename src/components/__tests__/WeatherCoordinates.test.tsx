...
describe('WeatherCoordinates', () => {
  ...
  test('Should navigate to Weather screen with given coordinates', async () => {
...
describe('Latitude field', () => {
  test('Should not show error when value is the lowest range value', () => {
    ...
    return expect(
      wrapper.findByText('Latitude must be a valide number'),
    ).rejects.toThrow();
  });
  ...
  test('Should show error when value is highter than the lowest range value', async () => {
    const wrapper = render(<WeatherCoordinates />);
    const field = wrapper.getByTestId('weather-coordinates-latitude');
    fireEvent.changeText(field, '91');
    await waitFor(() => {
  ...
  test('Should show error when value is not a number', async () => {
    const wrapper = render(<WeatherCoordinates />);
    const field = wrapper.getByTestId('weather-coordinates-latitude');
    fireEvent.changeText(field, 'some string');
    await waitFor(() => {
      wrapper.getByText('Latitude must be a valid number');
    });
  });
  ...
  describe('Longitude field', () => {
    test('Should not show error when value is the lowest range', () => {
      const wrapper = render(<WeatherCoordinates />);
      const field = wrapper.getByTestId('weather-coordinates-longitude');
      fireEvent.changeText(field, '-180');
      return expect(
        wrapper.findByText('Longitude must be a valide number'),
      ).rejects.toThrow();
    });
    test('Should not show error when value is the highest range', () => {
      const wrapper = render(<WeatherCoordinates />);
      const field = wrapper.getByTestId('weather-coordinates-longitude');
      fireEvent.changeText(field, '180');
      return expect(
